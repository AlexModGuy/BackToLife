...
public class EntityVelociraptor extends EntityLandPrehistoric {
	...
	public EntityVelociraptor(World worldIn) {
		...
		this.ANIMATION_EAT = Animation.create(20);
		this.ANIMATION_SPEAK = Animation.create(20);
	}
	...
	public void onLivingUpdate() {
		...
		if (this.getRidingEntity() != null && this.getRidingEntity() instanceof EntityLivingBase) {
			...
			if (attackTime < 80) {
				...
				if (attackTime % 40 == 0) {
					((EntityLivingBase) this.getRidingEntity()).attackEntityFrom(DamageSource.causeMobDamage(this), (float) ((int) this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue()));
				}
				if (attackTime % 10 == 0) {
	...
	public boolean isMovementCeased() {
		return this.isSleeping() || this.isSitting() || this.getAnimation() == this.ANIMATION_JUMP || isPouncing;
	}
	...
	protected void collideWithEntity(Entity entityIn) {
		if (this.getAttackTarget() != null && this.getAttackTarget() == entityIn && this.getAnimationTick() > 10) {
			if (this.width > entityIn.width) {
				this.startRiding(entityIn);
			} else if(entityIn instanceof EntityPlayer){
				((EntityPlayer) entityIn).attackEntityFrom(DamageSource.causeMobDamage(this), (float) ((int) this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue()));
			}
		}
